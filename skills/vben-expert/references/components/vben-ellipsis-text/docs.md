---
title: Vben EllipsisText
description: How to use the vben ellipsis-text component.
---
...
| 名称 | 路径 | 说明 |
|------|------|------|
| 基础用法 | [line](demo/line/index.vue) | 通过默认插槽提供文本内容，`maxWidth` 限制文本区域宽度。 |
| 可折叠文本 | [expand](demo/expand/index.vue) | 通过 `line` 设置折叠后的最大行数，通过 `expand` 开启点击展开与收起。 |
| 自定义提示浮层 | [tooltip](demo/tooltip/index.vue) | 通过 `tooltip` 插槽自定义提示内容。 |
| 仅在省略时显示 Tooltip | [auto-display](demo/auto-display/index.vue) | 通过 `tooltip-when-ellipsis` 控制仅在文本被截断时显示 Tooltip。 |
